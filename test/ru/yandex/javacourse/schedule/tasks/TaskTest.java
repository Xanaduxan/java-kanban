...
public class TaskTest {
    ...
    void deleteSubtaskCleansAll() {
        ...
        boolean found = false;
        for (Subtask st : taskManager.getSubtasks()) { if (st.getId() == s1) { found = true; break; } }
        assertFalse(found, "Not in storage");


        boolean idInEpic = false;
        for (int id : taskManager.getEpic(epicId).getSubtaskIds()) { if (id == s1) { idInEpic = true; break; } }
        assertFalse(idInEpic, "Not in epic");


        boolean inHistory = false;
        for (Task t : taskManager.getHistory()) { if (t.getId() == s1) { inHistory = true; break; } }
